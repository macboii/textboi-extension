...
- [x] 빈 상태 단축키 가이드 (C+C, ⌘↵)
- [x] Source lang: 🌐 Auto-detect 뱃지
- [x] Target lang: native `<select>`
- [x] Rewrite style: native `<select>` (6개 타입)
...
### 미구현 — 우선순위 높음 🔴
- [ ] Source lang: 커스텀 드롭다운 (검색 + 최근 항목) → `.tb-source-lang-btn`
- [ ] Target lang: 커스텀 드롭다운 (검색 + 최근 항목) → `.tb-target-lang-btn`
- [ ] Rewrite style: 커스텀 드롭다운 (검색 + 설명 + 커스텀 프롬프트) → `.tb-rewrite-btn`

### 미구현 — 우선순위 중간 🟡
- [ ] 토큰 카운터 (`0 / 10,000`) — 익스텐션 환경에서 tiktoken 없이 근사치 구현 필요
- [ ] 제출 버튼 (`⬇` 아이콘) — 텍스트박스 내 하단 우측
