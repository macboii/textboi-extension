...
// 게스트 잔여 횟수 조회 (읽기 전용, 카운트 변경 없음) — popup 비로그인 화면에서 호출
{ type: 'GET_GUEST_QUOTA' }
// → 응답: { quota: { ok, remaining, limitExceeded } }

// 게스트 배너 "Sign in" 버튼 클릭 → 팝업 강제 열기
{ type: 'TRIGGER_LOGIN' }
```
...
// → 응답: { ok: true, type: 'token_grant'|'plan_extension', value: number }
//        | { ok: false, error: 'INVALID_CODE'|'ALREADY_USED'|'EXPIRED'|'MAX_USES_REACHED'|'NOT_LOGGED_IN' }
...
익스텐션 UI는 **한국어/영어 이진 분기**만 지원한다. `chrome.i18n.getUILanguage()` → `navigator.language` → `navigator.languages` 순으로 체크해 어느 하나라도 `"ko"`로 시작하면 `_isKo = true`.

```javascript
// content.js 및 popup.js 최상단에 선언 (모듈 상수, 런타임 불변)
const _isKo = (() => {
  try { if ((chrome.i18n?.getUILanguage?.() || "").startsWith("ko")) return true; } catch {}
  if ((navigator.language || "").startsWith("ko")) return true;
  return (navigator.languages || []).some(l => l.startsWith("ko"));
})();
const _strings = { /* 한/영 UI 문자열 맵 */ };
```

- `content.js` → `_strings` (패널·버블·토스트 문자열)
- `popup.js` → `_p` (팝업 UI 문자열)
- `REWRITE_TYPES`에 `labelKo`/`descriptionKo` 필드가 있으며, `_isKo` 시 해당 필드 우선 사용

새 UI 문자열 추가 시 반드시 `_strings`/`_p` 객체에 한/영 쌍으로 추가. 코드에 하드코딩된 영문 문자열 직접 삽입 금지.
