...
| `SidePanel._quotaExceeded` | `boolean` | 토큰/게스트 한도 초과 시 true. `_rerun()` 진입 차단 + `state = "error"` 로 Cmd+Enter 차단. `show()` 호출 시 반드시 false로 리셋. |
...
- 장치 세션 미등록(`DEVICE_NOT_AUTHORIZED`): `ensureDeviceSessionOnce(token, deviceId)` — `chrome.storage`에 `tb_session_{deviceId}` 키로 캐시해 중복 호출 방지. 성공 시에만 캐시(`true`) 저장. `saveHistory()` 401 시 스탈 캐시를 `chrome.storage.local.remove(key)`로 제거 후 `registerDeviceSession()` 재호출 → 성공 시 캐시 재저장 → `doSave()` 재시도. `/save-session` 호출 시 반드시 `Authorization: Bearer {token}` 헤더 포함 (Worker JWT 미들웨어 통과 필수 — device session 미들웨어만 bypass, JWT 미들웨어는 bypass 아님).
